$(document).ready(function() {
    ...
    this.parseSlider = (res) => {
        ...
        if(res.crashed) 
        {
            ...
            $('.btn-withdraw span').text('Забрать');
        } else {
            if(!window.isCashout && window.withdraw > 0) $('.btn-withdraw span').text('Забрать ' + parseFloat(window.bet*parseFloat(res.float.toFixed(2))).toFixed(2));
    ...
    $('.btn-play').click(() => {
        $.ajax({
            ...
            success : res => {
                ...
                {
                    window.bet = res.bet;
                    $('.btn-withdraw span').text('Забрать ' + (window.bet).toFixed(2));
